...
<Note>
  Cancellation does not issue a refund. If the merchant wants to refund remaining time, they should call [`refund()`](/api-reference/refund) separately. The token allowance remains on-chain but becomes irrelevant since the contract will never call `transfer_from` on a cancelled subscription.
</Note>
