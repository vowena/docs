...
<Tip>
  This returns all subscriptions regardless of status. To display only active
  subscriptions in your UI, filter the results by checking each subscription's
  `status` field via [`get_subscription`](/api-reference/get-subscription).
</Tip>
